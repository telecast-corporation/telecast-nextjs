  ...
  return (
    <Container maxWidth="lg" sx={{ py: 10 }}>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Upload Episode to {selectedPodcast?.title}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            <TextField
              label="Episode Title"
            ...
            <TextField
              ...
              onChange={(e) => setUploadForm({ ...uploadForm, episodeNumber: e.target.value })}
            />
            <TextField
              label="Season Number"
              type="number"
              ...
              onChange={(e) => setUploadForm({ ...uploadForm, seasonNumber: e.target.value })}
            />
            <TextField
            ...
            {uploadForm.audioFile && (
              <Typography variant="body2" color="text.secondary">
                Selected: {uploadForm.audioFile.name}
              </Typography>
            )}
          </Box>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleEpisodeUpload}
            disabled={!uploadForm.title || !uploadForm.description || !uploadForm.audioFile || isUploading}
        ...
        <DialogTitle>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button
              ...
              onClick={() => {
                setEpisodesDialogOpen(false);
                handleUploadEpisode(selectedPodcast!);
              }}
            >
              Upload New Episode
            </Button>
              ...
              {episodes.map((episode) => (
                ...
                >
                      ...
                      <Box sx={{ mt: 1 }}>
                        ...
                        {episode.keywords && episode.keywords.length > 0 && (
                          <Box sx={{ mt: 1, display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                            {episode.keywords.map((keyword: string) => (
                              <Chip
                                key={keyword}
                                label={keyword}
                                size="small"
                  ...
                  <ListItemSecondaryAction>
                    <IconButton 
                      edge="end" 
                      aria-label={currentlyPlaying === episode.id ? "pause" : "play"}
                      onClick={() => handlePlayEpisode(episode)}
                      color={currentlyPlaying === episode.id ? "primary" : "default"}
                    >
                      {currentlyPlaying === episode.id ? <PauseIcon /> : <PlayArrowIcon />}
                    </IconButton>
                  </ListItemSecondaryAction>
                </ListItem>
              ))}
