...
async function searchTvShows(query: string, maxResults: number = 30) {
    console.log('🔍 searchTvShows CALLED! Query:', query, 'MaxResults:', maxResults);

    const shows = await searchOMDBTvShows(query, maxResults);

    console.log('🔍 OMDB returned', shows.length, 'shows');
    console.log('🔍 First show data:', JSON.stringify(shows[0], null, 2));
    ...
    console.log('🔍 Mapped TV shows:', mapped.length);
    console.log('🔍 First mapped show:', JSON.stringify(mapped[0], null, 2));
    ...
    try {
        ...
        if (trending && query === 'recommended') {
            ...
            if (types.includes('tv')) {
                console.log('📺 Falling back to popular TV shows');
                const fallbackResults = await searchTvShows('popular', Math.min(maxResults, 300));
                // ... pagination logic
            }
            ...
            try {
                ...
                if (types.includes('all')) {
                    ...
                    if (types.includes('tv')) trendingResults.push(...(trendingData.tvshows || []));  // ✅ Add this
