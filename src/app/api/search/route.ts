...
interface SearchResult {
  ...
  category?: string;
  tags?: string[];
  type: 'podcast' | 'video' | 'music' | 'book' | 'audiobook' | 'tvshow';
}
...
async function searchTvShows(query: string, maxResults: number = 30) {
    ...
    return shows.map((show: any) => ({
        ...
        url: `/video/${show.id}`, // Using /video for now, can be changed to /tv
        author: "N/A", // Director doesn't apply well to series
