  ...
  try {
    ...
    if (jsonLdMatch && jsonLdMatch[1]) {
      try {
        ...
        const audiobookData = Array.isArray(jsonData) ? jsonData.find((item: any) => item['@type'] === 'Audiobook') : (jsonData['@type'] === 'Audiobook' ? jsonData : null);

        if (audiobookData) {
          console.log('✅ Found and parsed JSON-LD data.');
          const author = Array.isArray(audiobookData.author) ? audiobookData.author.map((a: { name: string }) => a.name).join(', ') : audiobookData.author?.name;
          const narrator = Array.isArray(audiobookData.readBy) ? audiobookData.readBy.map((a: { name: string }) => a.name).join(', ') : audiobookData.readBy?.name;
